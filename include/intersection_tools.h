...
// Deprecation
//bool is_visible(const Eigen::Vector3f& v_view_pos, const Eigen::Vector3f& v_view_direction, const Eigen::Vector3f& v_point_pos,const Tree& v_tree,const float fov_in_degree_h, const float fov_in_degree_v, float max_distance=-1.f);
bool inside_frustum(
    ...
    const float v_max_distance = -1.f
);
bool is_visible(const Eigen::Matrix3f& v_intrinsic_matrix, const Eigen::Isometry3f& v_camera_matrix, const Eigen::Vector3f& v_view_pos ,const Eigen::Vector3f& v_point_pos, 
