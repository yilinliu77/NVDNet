	...
	for (auto& item_mtl : total_mtls)
	{
		if(item_mtl.diffuse_texname!="")
		{
			...
			boost::filesystem::copy_file(item_mtl.diffuse_texname,
				new_path,boost::filesystem::copy_options::overwrite_existing);
...
// #pragma omp parallel for reduction(+:total_area)
	...
	return sample_points(v_mesh, std::ceil(v_num_points_per_m2 * total_area), true);
}

Point_set sample_poisson_points(const std::vector<Triangle_3>& v_mesh, const int v_num_points, const int init_factor)
{
	using namespace vcg;
	class MyEdge;
	class MyFace;
	...
	MyMesh m;
	tri::Allocator<MyMesh>::AddVertices(m, v_mesh.size() * 3);
	tri::Allocator<MyMesh>::AddFaces(m, v_mesh.size());
	// #pragma omp parallel for
	for (int i_face = 0; i_face < v_mesh.size(); ++i_face)
	{
		m.vert[i_face * 3 + 0].P() = Point3d(v_mesh[i_face][0].x(), v_mesh[i_face][0].y(), v_mesh[i_face][0].z());
		m.vert[i_face * 3 + 1].P() = Point3d(v_mesh[i_face][1].x(), v_mesh[i_face][1].y(), v_mesh[i_face][1].z());
		m.vert[i_face * 3 + 2].P() = Point3d(v_mesh[i_face][2].x(), v_mesh[i_face][2].y(), v_mesh[i_face][2].z());

		m.face[i_face].V(0) = &m.vert[i_face * 3 + 0];
		m.face[i_face].V(1) = &m.vert[i_face * 3 + 1];
		m.face[i_face].V(2) = &m.vert[i_face * 3 + 2];
		Vector_3 normal = CGAL::cross_product(v_mesh[i_face][1] - v_mesh[i_face][0], v_mesh[i_face][2] - v_mesh[i_face][0]);
		m.face[i_face].N() = Point3d(normal.x(), normal.y(), normal.z()).normalized();
	}

	Point_set montecarlo_points = sample_points(v_mesh, std::max(10000, v_num_points * init_factor));
	...
	double radius = 0.;
	tri::PoissonPruningExact<MyMesh>(
		MontecarloMesh, poissonSamplesVP, radius, v_num_points);

	Point_set p(true);
	auto index_map = p.add_property_map<int>("face_index", 0).first;
	p.resize(std::min(v_num_points, (int)poissonSamplesVP.size()));
	
	// #pragma omp parallel for
	for (int i_point = 0; i_point < std::min(v_num_points, (int)poissonSamplesVP.size()); ++i_point)
	{
		int index = poissonSamplesVP[i_point] - &MontecarloMesh.vert[0];
		index_map[i_point] = montecarlo_index_map[index];

		p.point(i_point) = Point_3(
			poissonSamplesVP[i_point]->P()[0],
			poissonSamplesVP[i_point]->P()[1],
			poissonSamplesVP[i_point]->P()[2]
		);
		p.normal(i_point) = Vector_3(
	...
	if (p.number_of_points() < v_num_points)
	{
		// LOG(INFO) << "Not enough sampling points, use montecarlo points to pad";
		const int num_complement_points = v_num_points - p.number_of_points();
		for(int i=0; i < num_complement_points;++i)
		{
			int idx = rand() % montecarlo_points.size();
			index_map[*p.insert(montecarlo_points.point(idx),montecarlo_points.normal(idx))] = montecarlo_index_map[idx];
		}
	}

	// CGAL::IO::write_point_set("sampled_points.ply", p);
	return p;
}

Point_set sample_points(const std::vector<Triangle_3>& v_mesh, const int v_num_points, bool at_least_one_point)
{
	std::mt19937 gen; std::uniform_real_distribution<double> dist(0.0f, 1.0f);
	Point_set o_point_set(true);
...
// #pragma omp parallel for
	for(int i_face = 0;i_face<v_mesh.size();++i_face)
	{
		...
		if (at_least_one_point)
			num_face_samples = std::max((uint)1, num_face_samples);
		else
		{
			if (dist(gen) < face_samples - static_cast<double>(num_face_samples)) 
			{
...
{
	double total_area = CGAL::Polygon_mesh_processing::area(v_mesh);
	return sample_points(v_mesh, std::ceil(v_num_points_per_m2 * total_area));
}
