...
)
{
	...
	// Output
	std::vector<double> bvh_distances(query_points.size()), bvh_closest_points(query_points.size() * 3), bvh_closest_bcs(query_points.size() * 3);
	std::vector<long long> bvh_closest_faces(query_points.size());
	...
	for (int i_point = 0; i_point < query_points.size(); ++i_point)
	{
		...
		Eigen::Vector3d v1 = cgal_2_eigen_point<double>(v_triangles[target_face].vertex(1)) - cgal_2_eigen_point<double>(v_triangles[target_face].vertex(0));
		Eigen::Vector3d v2 = cgal_2_eigen_point<double>(v_triangles[target_face].vertex(2)) - cgal_2_eigen_point<double>(v_triangles[target_face].vertex(0));
